...
// SOLUSI 1: Disable GPU Acceleration
app.commandLine.appendSwitch('disable-gpu')
app.commandLine.appendSwitch('disable-software-rasterizer')

// SOLUSI 3: Masalah Sertifikat
...
function createWindow() {
    mainWindow = new BrowserWindow({
        ...
        webPreferences: {
            ...
            webSecurity: true,
            spellcheck: false,
            // SOLUSI 2: Tambahan webPreferences untuk mengatasi CSP dan CORS
            webviewTag: true,
...
ipcMain.on('check-session-and-navigate-back', () => {
    ...
    if (currentSession) {
        // SOLUSI 4: Penanganan error yang lebih baik
        mainWindow.loadURL(currentSession['URL Ujian'])
            .catch((error) => {
                console.error('Error loading exam URL:', error);
                if (!mainWindow.isDestroyed() && !mainWindow.webContents.isDestroyed()) {
                    // Tampilkan halaman error dengan informasi lebih detail
                    mainWindow.loadFile('src/connection-error.html');
                    mainWindow.webContents.once('did-finish-load', () => {
                        mainWindow.webContents.executeJavaScript(`
                            const errorDiv = document.createElement('div');
                            errorDiv.innerHTML = '<p>Detail Error: ${error.message.replace(/'/g, "\\'")}</p>';
...
ipcMain.on('load-exam', () => {
    ...
    // SOLUSI 4: Penanganan error yang lebih baik saat memuat URL ujian
    mainWindow.loadURL(currentSession['URL Ujian'])
        .catch((error) => {
...
ipcMain.on('retry-connection', async () => {
    ...
    try {
        ...
        if (currentSession) {
            ...
            mainWindow.loadURL(currentSession['URL Ujian'])
                .catch((error) => {
                    console.error('URL loading error on retry:', error);
                    if (!mainWindow.isDestroyed() && !mainWindow.webContents.isDestroyed()) {
                        mainWindow.loadFile('src/connection-error.html');
                        mainWindow.webContents.once('did-finish-load', () => {
                            mainWindow.webContents.executeJavaScript(`
        ...
        // Jika masih gagal, tetap di halaman error dengan detail
        if (!mainWindow.isDestroyed() && !mainWindow.webContents.isDestroyed()) {
            mainWindow.loadFile('src/connection-error.html');
            mainWindow.webContents.once('did-finish-load', () => {
                mainWindow.webContents.executeJavaScript(`
